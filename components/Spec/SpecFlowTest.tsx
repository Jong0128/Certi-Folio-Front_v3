...
import { SpecReport } from './SpecReport'; // Import SpecReport
...
export const SpecFlowTest: React.FC = () => {
  ...
  const handleEtcComplete = () => {
      setUserData(prev => ({ ...prev, solvedAcId: solvedIdInput }));
      setStage('finished');
  };
            ...
            {mainCategories.map((cat, index) => {
                ...
                return (
                    <div key={cat.id} className="flex flex-col items-center gap-2 relative px-2">
                        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 border-2 ${
                            status === 'completed' ? 'bg-cyan-500 border-cyan-500 text-white shadow-md' :
                        ...
                        <span className={`text-xs md:text-sm font-medium whitespace-nowrap ${
                             status === 'active' ? 'text-cyan-700 font-bold' : 
