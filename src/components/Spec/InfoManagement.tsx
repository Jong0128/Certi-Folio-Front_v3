...
import { portfolioApi, userApi, codingTestApi } from "../../api/userApi";
...
export const InfoManagement: React.FC = () => {
  ...
  const [addingMode, setAddingMode] = useState<
    "projects" | "activities" | "certificates" | "career" | null
  >(null);
      ...
      try {
        ...
        setUserData((prev) => {
          ...
          if (profileRes.status === "fulfilled" && profileRes.value) {
            const profile =
              (profileRes.value as any).data?.data ||
              (profileRes.value as any).data;
            if (profile) {
          ...
          if (educations.status === "fulfilled" && educations.value) {
            ...
            updated.academicStatus = edu.status || prev.academicStatus;
    ...
    try {
      ...
      if (userData.schoolName) {
        await portfolioApi.saveEducation({
          ...
          gpa: userData.gpa,
          maxGpa: userData.maxGpa,
        });
      }

      // Sync Solved.ac to Backend
      if (userData.solvedAcId) {
        await codingTestApi.saveCodingTest(userData.solvedAcId);
      }
  ...
  const handleActivityAdd = async (newActivity: ActivityData) => {
    const updatedActivities = [...userData.activities, newActivity];
  ...
  const handleCareerAdd = async (newCareer: CareerData) => {
    const updatedCareers = [...userData.careers, newCareer];
          ...
          <div className="max-w-2xl mx-auto py-4 animate-fade-in-up">
            ...
            <div className="bg-white/50 p-8 rounded-3xl border border-white/60 shadow-sm space-y-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleChange("academicStatus", e.target.value)
                    }
                  >
              ...
              <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                <label className="text-gray-400 text-xs font-bold uppercase tracking-wider ml-1 mb-2 block text-center">
                  전체 평점 (GPA)
                </label>
                <div className="flex items-center justify-center gap-3">
                  <input
                    ...
                    className="w-24 text-center bg-white border border-gray-200 rounded-lg py-2 font-bold text-gray-900 text-xl focus:border-cyan-500 outline-none"
                    placeholder="0.0"
                  />
                  ...
                  <input
                    type="number"
                    step="0.1"
                    value={userData.maxGpa}
                    onChange={(e) => handleChange("maxGpa", e.target.value)}
                    className="w-24 text-center bg-white border border-gray-200 rounded-lg py-2 font-bold text-gray-900 text-xl focus:border-cyan-500 outline-none"
                    placeholder="4.5"
                  />
                </div>
            ...
            <div className="flex justify-center pt-8">
              <Button
                ...
                className="w-full max-w-sm py-4 rounded-xl shadow-lg shadow-gray-200"
              >
                저장하기
              </Button>
            </div>
            ...
            <div className="space-y-6">
              {userData.projects && userData.projects.length > 0 ? (
                userData.projects.map((proj, idx) => (
                  <div
                    key={idx}
                    className="group relative bg-white border border-gray-200 rounded-2xl p-6 hover:border-cyan-300 hover:shadow-lg hover:shadow-cyan-500/5 transition-all duration-300"
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          <h4 className="font-bold text-lg text-gray-800 group-hover:text-cyan-700 transition-colors">
                            {proj.projectName || "프로젝트명 없음"}
                          </h4>
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          ...
                          onClick={() => {
                            const newProjects = userData.projects.filter(
                              (_, i) => i !== idx,
                            );
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {proj.techStack?.map((stack, i) => (
                        <span
                          key={i}
                          className="text-xs px-2 py-1 rounded-md bg-cyan-50 text-cyan-700 font-medium border border-cyan-100"
                        >
                          {stack}
                        </span>
                      ))}
            ...
            <div className="space-y-6">
              {userData.activities && userData.activities.length > 0 ? (
                userData.activities.map((act, idx) => (
                  <div
                    key={idx}
                    className="group relative bg-white border border-gray-200 rounded-2xl p-6 hover:border-purple-300 hover:shadow-lg hover:shadow-purple-500/5 transition-all duration-300"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="font-bold text-lg text-gray-800 group-hover:text-purple-700 transition-colors">
                          {act.activityName || "활동명 없음"}
                        </h4>
                        <span className="text-xs font-medium text-purple-600 bg-purple-50 px-2 py-0.5 rounded-md mt-1 inline-block">
                          {act.role}
                        </span>
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"
                          onClick={() => {
                            const newActs = userData.activities.filter(
                              (_, i) => i !== idx,
                            );
                ...
                <div className="text-center py-20 bg-gray-50/50 rounded-3xl border border-dashed border-gray-300">
                  ...
                  <button
                    onClick={() => setAddingMode("activities")}
                    className="mt-4 text-purple-600 font-bold hover:underline text-sm"
                  >
                    활동 추가하기
                  </button>
                </div>
            ...
            <div className="space-y-4">
              {userData.certificates && userData.certificates.length > 0 ? (
                userData.certificates.map((cert, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-5 bg-white border border-gray-200 rounded-2xl shadow-sm hover:border-orange-300 transition-all hover:translate-x-1 group"
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl shadow-inner ${idx % 2 === 0 ? "bg-orange-50 text-orange-500" : "bg-blue-50 text-blue-500"}`}
                      >
                        {idx % 2 === 0 ? "📄" : "🗣️"}
                      </div>
                      <div>
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        onClick={() => {
                          const newCerts = userData.certificates.filter(
                            (_, i) => i !== idx,
                          );
                ...
                <div className="text-center py-20 bg-gray-50/50 rounded-3xl border border-dashed border-gray-300">
                  ...
                  <button
                    onClick={() => setAddingMode("certificates")}
                    className="mt-4 text-orange-600 font-bold hover:underline text-sm"
                  >
                    자격증 추가하기
                  </button>
                </div>
                    ...
                    <div className="flex-1 bg-white border border-gray-200 rounded-2xl p-6 hover:border-green-300 hover:shadow-lg hover:shadow-green-500/5 transition-all mb-4 relative">
                      <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                        ...
                        <button
                          ...
                          onClick={() => {
                            const newCar = userData.careers.filter(
                              (_, i) => i !== idx,
                            );
                      ...
                      <h4 className="font-bold text-xl text-gray-900 mb-0.5">
                        {career.companyName}
                      </h4>
            ...
            <div className="bg-white/60 p-8 rounded-3xl border border-white/60 shadow-sm space-y-8">
              <div className="bg-gradient-to-br from-[#2dce89]/10 to-[#2dce89]/5 p-6 rounded-2xl border border-[#2dce89]/20 relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <p className="text-sm text-gray-600 mb-6 leading-relaxed">
                    백준(BOJ) 계정을 연동하면 알고리즘 문제 해결 역량을
                    <br />
              ...
              <div className="pt-4">
                <Button
                  variant="primary"
                  onClick={handleManualSave}
                  className="w-full py-4 text-lg font-bold shadow-lg shadow-gray-200"
                >
                  설정 저장하기
                </Button>
              </div>
            ...
            <nav className="flex flex-col space-y-2">
              {categories.map((cat) => (
                <button
                  ...
                  className={`flex items-center gap-3 px-4 h-12 w-full rounded-xl transition-all duration-200 text-left ${
                    activeTab === cat.id
            ...
            <div className="flex overflow-x-auto no-scrollbar gap-2 px-1">
              {categories.map((cat) => (
                <button
                  ...
                  className={`flex items-center justify-center px-4 py-2 rounded-xl whitespace-nowrap text-sm font-bold transition-all ${
                    activeTab === cat.id
        ...
        <div className="w-full xl:col-start-2 min-w-0">
          {addingMode ? (
            <div className="animate-fade-in-up">
              {addingMode === "projects" && (
                <ProjectFlow
                  onComplete={handleProjectAdd}
                  onBack={() => setAddingMode(null)}
                />
              )}
              {addingMode === "activities" && (
                <ActivityFlow
                  onComplete={handleActivityAdd}
                  onBack={() => setAddingMode(null)}
                />
              )}
              {addingMode === "certificates" && (
                <CertificateFlow
                  onComplete={handleCertAdd}
                  onBack={() => setAddingMode(null)}
                />
              )}
              {addingMode === "career" && (
                <CareerFlow
                  onComplete={handleCareerAdd}
                  onBack={() => setAddingMode(null)}
                />
              )}
            </div>
